...
void UTAbilitySystemLibrary::InitializedDefaultAttributes(const UObject* WorldContextObject,
                                                          ...
                                                          UAbilitySystemComponent* ASC)
{
	...
	FGameplayEffectSpecHandle PrimarySpecHandle = ASC->MakeOutgoingSpec(ClassDefaultInfo.PrimaryAttributes, Level,
	...
	const FGameplayEffectSpecHandle SecondarySpecHandle = ASC->MakeOutgoingSpec(
		CharacterClassDataAsset->SecondaryAttributes, Level, SecondaryAttributesContextHandle);
	...
	const FGameplayEffectSpecHandle VitalSpecHandle = ASC->MakeOutgoingSpec(
		CharacterClassDataAsset->VitalAttributes, Level, VitalAttributesContextHandle);
	ASC->ApplyGameplayEffectSpecToSelf(*VitalSpecHandle.Data.Get());
}

void UTAbilitySystemLibrary::GiveCommonAbilities(const UObject* WorldContextObject, UAbilitySystemComponent* ASC)
{
	ATGameModeBase* TGameMode = Cast<ATGameModeBase>(UGameplayStatics::GetGameMode(WorldContextObject));
	if (TGameMode == nullptr)
	{
		return;
	}
	...
	for (TSubclassOf<UGameplayAbility> AbilityClass : CharacterClassDataAsset->CommonAbilities)
	{
		if (ASC->GetAvatarActor()->Implements<UCharacterData>())
		{
			FGameplayAbilitySpec AbilitySpec = FGameplayAbilitySpec(AbilityClass,
			                                                        ICharacterData::Execute_GetPlayerLevel(
				                                                        ASC->GetAvatarActor()));
...
int32 UTAbilitySystemLibrary::GetXPRewardForClassAndLevel(const UObject* WorldContextObject,
